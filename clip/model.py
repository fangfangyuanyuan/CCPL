...
class CrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads):
        ...
        self.proj_q1 = nn.Linear(embed_dim, self.k_dim * num_heads, bias=False)
        self.proj_k2 = nn.Linear(embed_dim, self.k_dim * num_heads, bias=False)
        self.proj_v2 = nn.Linear(embed_dim, self.v_dim * num_heads, bias=False)
        self.proj_o = nn.Linear(self.v_dim * num_heads, embed_dim)

    def forward(self, x1, x2, mask=None):  # # [4,213,768]  [10,77,512] [1,4,512][1,10,512]
        batch_size, seq_len1, in_dim1 = x1.size()
        seq_len2 = x2.size(1)

        q1 = self.proj_q1(x1).view(batch_size, seq_len1, self.num_heads, self.k_dim).permute(0, 2, 1,
                                                                                             3)  # [1,8,4,512][4,8,213,512]
        k2 = self.proj_k2(x2).view(batch_size, seq_len2, self.num_heads, self.k_dim).permute(0, 2, 3,
                                                                                             1)  # [1,8,512,10][10,8,512,77]
        v2 = self.proj_v2(x2).view(batch_size, seq_len2, self.num_heads, self.v_dim).permute(0, 2, 1,
                                                                                             3)  # [1,8,10,512][10,8,77,512]

        attn = torch.matmul(q1, k2) / self.k_dim ** 0.5  # [1,8,4,512]
        # if mask is not None:
        #     attn = attn.masked_fill(mask == 0, -1e9)
        attn = F.softmax(attn, dim=-1)
        output = torch.matmul(attn, v2).permute(0, 2, 1, 3).contiguous().view(batch_size, seq_len1, -1)  # [1,4,512]
        output = self.proj_o(output)  # [1,4,512]

        return output


class ResidualAttentionBlock_CMPA(nn.Module):
    def __init__(self, d_model_visual: int, n_head_visual: int, d_model_text: int, n_head_text: int, prompt_attn=None,
                 attn_mask_visual: torch.Tensor = None,
    ...
    def forward(self, inputs):
        ...
        if not self.first_layer:
            if compound_prompts_depth > 0:
                ...
                # print(f"vision_layers:{self.vision_layers}")
                # print(f"i:{self.i}")
...
class CMPA(nn.Module):
    def __init__(self,
        ...
        if self.parameters_sharing:
            prompt_attn = PromptAttention(vision_width, vision_heads, transformer_width, transformer_heads,
                                          design_details)
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_CMPA(vision_width, vision_heads, transformer_width, transformer_heads,
                                              prompt_attn,
                                              attn_mask_visual=None, attn_mask_text=attn_mask_text,
                                              design_details=design_details, i=i)
        ...
        #############################添加部分
        # self.last_attention = nn.MultiheadAttention(embed_dim, transformer_heads)  # 512 8,每个头是64
        self.last_attention = CrossAttention(embed_dim, transformer_heads)  # 512 8,每个头是64
        # self.last_proj_v2t = nn.Linear(vision_width, transformer_width)
        # self.last_proj_t2v = nn.Linear(transformer_width, vision_width)
        self.last_1n = LayerNorm(embed_dim)
        # self.last_1n_text = LayerNorm(transformer_width)
        self.last_mlp_text = nn.Sequential(OrderedDict([
            ("c_fc_1", nn.Linear(embed_dim, embed_dim * 4)),
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(embed_dim * 4, embed_dim))
        ]))
        # self.last_mlp_visual = nn.Sequential(OrderedDict([
        #     ("c_fc_1", nn.Linear(embed_dim, embed_dim * 4)),
        #     ("gelu", QuickGELU()),
        #     ("c_proj", nn.Linear(embed_dim * 4, embed_dim))
        # ]))

        self.last_gelu = QuickGELU()
