...
def parse_function(*metrics, directory="", args=None, end_signal=None):
    print(f"Parsing files in {directory}")  #output/eurosat/1shots/
    subdirs = listdir_nohidden(directory, sort=True) # seed1,seed2,seed3

    outputs = []

    for subdir in subdirs:
        fpath = osp.join(directory, subdir, "log.txt")  #output/eurosat/1shots/seed2/log.txt
        ...
        with open(fpath, "r") as f:
            ...
            for line in lines:
                line = line.strip()
                # print(f"{line}&&&&&&&&&&& {end_signal}:############{line == end_signal}#")
                if line == end_signal:  # Finish training
                    good_to_go = True

                for metric in metrics:
                    match = metric["regex"].search(line)
                    # print(f"{match} and {good_to_go}:#############################")
