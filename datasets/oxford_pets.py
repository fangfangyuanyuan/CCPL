    ...
    def read_split(filepath, path_prefix):
        def _convert(items):
            ...
            for impath, label, classname in items:
                ...
                # impath: /home/zfy/auto-tmp/cmpa/data/eurosat/2750/SeaLake/SeaLake_1034.jpg
                # label:9
                # classname:Sea or Lake
                item = Datum(impath=impath, label=int(label), classname=classname)
                out.append(item)
            return out

        print(f"Reading split from {filepath}")
        split = read_json(filepath) #split:3
        train = _convert(split["train"]) # 13500
        val = _convert(split["val"]) # 5400
        test = _convert(split["test"]) #8100
