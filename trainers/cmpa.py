...
class Encoder(nn.Module):
    ...
    def forward(self, x_visual, visual_ctx, prompts_text, tokenized_prompts, compound_prompts_depth):
        ...
        combined = [x_visual, x_text, compound_prompts_depth,
                    0]  # third argument is the counter which denotes depth of prompt
...
################添加部分
class LastCrossAttentin(nn.Module):
        def __init__(self, clip_model, classnames):
            super().__init__()
            self.last_attention = clip_model.last_attention  # 512 8
            self.n_cls = len(classnames)
            self.last_1n = clip_model.last_1n
            self.last_1n_1 = LayerNorm(self.n_cls)

            self.mlp_last = nn.Sequential(OrderedDict([
                ("last_fc", nn.Linear(clip_model.embed_dim, clip_model.embed_dim * 4)),
                ("gelu", clip_model.last_gelu),
                ("last_proj", nn.Linear(clip_model.embed_dim * 4, clip_model.embed_dim))
            ]))

            self.last_linear = nn.Linear(clip_model.embed_dim, self.n_cls)
            self.sigmoid = nn.Sigmoid()
            # self.last_gelu = clip_model.last_gelu
        # [4,213,768] [10,77,512]->[4->10]     #[4,512] [10,512]
        def forward(self, x_visual, x_text):  #
            x_visual = x_visual.unsqueeze(0)  # [1,4,512]
            x_text = x_text.unsqueeze(0)  # [1,10,512]

            text_cross_output =  self.last_attention(self.last_1n(x_visual),  # [1,4,512]
                                                    self.last_1n(x_text),  # [1,10,512]
                                                    self.last_1n(x_text))[0] # [1,10,512]
            # text_cross_output = self.last_1n_1(self.last_linear(text_cross_output.squeeze(0).float())) #[4,512]->[4,10]
            text_cross_output = text_cross_output.squeeze(0).float() #[1,4,512]->[4,512]
            cross_output = text_cross_output + self.mlp_last(self.last_1n(text_cross_output)) #[4,512]
            cross_output = self.last_linear(self.last_1n(cross_output)) #[4,512]
            cross_output = self.sigmoid(cross_output)
            return cross_output
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.dtype = clip_model.dtype
        self.last_cross_attention = LastCrossAttentin(clip_model, classnames)
    ...
    def forward(self, image, label=None):
        ...
        image_features, text_features = self.encoder(image.type(self.dtype), prompts_visual, prompts_text,
                                                     tokenized_prompts, compound_prompts_depth)

        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        logits = logit_scale * image_features @ text_features.t()
        #################修改  # [4,512] [512,10] ->[4,10][10,512]->[4,512]->[4,10]
        # y_prob = self.last_cross_attention(image_features, text_features)  # [4,512] [10,512]  [4,213,768] [10,77,512]
        #################修改

        if self.prompt_learner.training:
            loss_itc = F.cross_entropy(logits, label)
            # label_itm = torch.zeros_like(y_prob)  # [4,10]
            # # one-hot形式
            # label_itm[torch.arange(label_itm.shape[0]), torch.tensor([l for l in label])] = 1
            # # print(f"y_prob:{y_prob}")
            # loss_itm = - torch.sum(label_itm * torch.log(y_prob) + (1 - label_itm) * torch.log(1 - y_prob)).mean()

            # loss_itm = F.cross_entropy(y_prob, label)
        ...
        for name, param in self.model.named_parameters():
            if name_to_update not in name:
                ...
                if "VPT" in name:
                    ...
                    param.requires_grad_(True)
                elif 'last_cross_attention' in name:
                    param.requires_grad_(True)
                else:
